...
games = []
games.append(game.Game(7, 6, 4, agent.InteractiveAgent("random"), aba.AlphaBetaAgent("alphabeta")))
...
for game in games:
    ...
    outcome = game.go()
    if player == 1:
        max_times.append(game.players[0].max_time)
    else:
        max_times.append(game.players[1].max_time)
    if outcome == player:
        wins += 1
    elif outcome == 0:
        ties += 1
...
print("AI had a max time of: " + str(max(max_times)))
